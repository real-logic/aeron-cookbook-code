...
public class NewTimerCommand {
   ...
   * The byte offset in the byte array for this LONG. Byte length is 8.
   */
  private static final int DEADLINE_OFFSET = 16;
  ...
  /**
   * Reads deadline as stored in the buffer.
   */
  public long readDeadline() {
    return buffer.getLong(initialOffset + DEADLINE_OFFSET, java.nio.ByteOrder.LITTLE_ENDIAN);
  }

  /**
   * Writes deadline to the buffer. Returns true if success, false if not.
   * @param value Value for the deadline to write to buffer.
   */
  public boolean writeDeadline(long value) {
    if (!isMutable) throw new RuntimeException("Cannot write to immutable buffer");
    mutableBuffer.putLong(initialOffset + DEADLINE_OFFSET, value, java.nio.ByteOrder.LITTLE_ENDIAN);
    return true;
  }
