...
public class RequestMethod {
   ...
   */
  public static final short EIDER_ID = 34;
   ...
   */
  private static final int PARAMETER_OFFSET = 44;
  ...
  /**
   * Reads parameter as stored in the buffer.
   */
  public String readParameter() {
    return buffer.getStringWithoutLengthAscii(initialOffset + PARAMETER_OFFSET, 50).trim();
  }

  /**
   * Writes parameter to the buffer. Returns true if success, false if not.Warning! Does not pad the string.
   * @param value Value for the parameter to write to buffer.
   */
  public boolean writeParameter(String value) {
    if (!isMutable) throw new RuntimeException("Cannot write to immutable buffer");
    if (value.length() > 50) throw new RuntimeException("Field parameter is longer than maxLength=50");
    mutableBuffer.putStringWithoutLengthAscii(initialOffset + PARAMETER_OFFSET, value);
    return true;
  }

  /**
   * Writes parameter to the buffer with padding. 
   * @param value Value for the parameter to write to buffer.
   */
  public boolean writeParameterWithPadding(String value) {
    final String padded = String.format("%50s", value);
    return writeParameter(padded);
  }
