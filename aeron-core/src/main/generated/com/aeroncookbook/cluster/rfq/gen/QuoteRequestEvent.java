...
public class QuoteRequestEvent {
   ...
   * The byte offset in the byte array for this INT. Byte length is 4.
   */
  private static final int BROADCASTEXCLUDEUSERID_OFFSET = 45;
  ...
  /**
   * Reads broadcastExcludeUserId as stored in the buffer.
   */
  public int readBroadcastExcludeUserId() {
    return buffer.getInt(initialOffset + BROADCASTEXCLUDEUSERID_OFFSET, java.nio.ByteOrder.LITTLE_ENDIAN);
  }

  /**
   * Writes broadcastExcludeUserId to the buffer. Returns true if success, false if not.
   * @param value Value for the broadcastExcludeUserId to write to buffer.
   */
  public boolean writeBroadcastExcludeUserId(int value) {
    if (!isMutable) throw new RuntimeException("Cannot write to immutable buffer");
    mutableBuffer.putInt(initialOffset + BROADCASTEXCLUDEUSERID_OFFSET, value, java.nio.ByteOrder.LITTLE_ENDIAN);
    return true;
  }
